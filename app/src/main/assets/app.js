(() => {
  ...
  function updateNotificationStatus() {
    if (window.AndroidBridge?.isNative?.()) {
      elements.notificationStatus.textContent = '系统接管';
      elements.notificationStatus.classList.add('is-on');
      elements.enableNotificationsButton.textContent = '已由系统接管';
  ...
  function vibrateDevice(pattern = VIBRATION_PATTERN) {
    ...
    if (window.AndroidBridge?.vibrate) {
      window.AndroidBridge.vibrate(Math.max(...pattern) || 360);
      return;
    }
  ...
  async function showSystemNotification(title, options) {
    if (window.AndroidBridge?.isNative?.()) return true;
  ...
  async function testNotification() {
    if (window.AndroidBridge?.showTest) {
      window.AndroidBridge.showTest();
      vibrateDevice();
      showToast('测试提醒已发送');
      return;
    }
  ...
  function cancelTaskNotification(taskId) {
    ...
    if (window.AndroidBridge?.cancelAlarm) {
      window.AndroidBridge.cancelAlarm(taskId);
      return;
    }
  ...
  function openTaskCalendar(task) {
    if (window.AndroidBridge?.openCalendar) {
      const due = parseDate(task.dueAt) || new Date();
      const end = new Date(due.getTime() + 30 * 60_000);
      closeDialog(elements.calendarDialog);
      window.AndroidBridge.openCalendar(task.title, task.notes || '', due.getTime(), end.getTime());
      showToast('正在打开系统日历');
      return;
    }
  ...
  async function refreshBackgroundReminderState() {
    if (window.AndroidBridge?.isNative?.()) {
      ...
      elements.backgroundButtonTitle.textContent = '系统闹钟已接管';
      elements.backgroundButtonSubtitle.textContent = '应用关闭后也会按时提醒';
      return;
    }
  ...
  async function registerServiceWorker() {
    if (window.AndroidBridge?.isNative?.()) return;
